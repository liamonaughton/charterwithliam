...
| N11 | 🟡 | P0 | compliance | **HubSpot now owns lead capture + follow-up email (DECISIONS D4).** Resend's welcome/guide flow no longer runs. **Action:** confirm the HubSpot follow-up email carries a working unsubscribe link + real physical mailing address (CLAUDE.md rule 4). Also: leads no longer write to Supabase — old `LeadForm`/`subscribe.ts`/Resend left unused for revert; decide later whether to remove. **DNS guardrail:** apex/`www` must stay on Vercel, never repoint to HubSpot. |
